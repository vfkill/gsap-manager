...
(function () {
    ...
    // a ser calculado relativo ao wrapper transformado (que se move). O
    ...
    //   end:   "bottom top"     — solta quando posição natural do bottom passa
    //                             pelo topo da viewport (= elemento totalmente
    //                             coberto pela seção abaixo)
    //   pinSpacing: false       — sem padding extra; conteúdo abaixo sobe e
    //                             cobre o elemento pinado (OVERLAP)
    //   refreshPriority: -1     — pin refresh antes de outros triggers, evita
    //                             desalinhamento quando há muitos ScrollTriggers
    //   invalidateOnRefresh: true — recalcula em resize/refresh
    //
    // Atributos opcionais:
    //   data-gsap-start        → override do start       (ex: "top 10%")
    //   data-gsap-end          → override do end         (ex: "+=200%", "bottom center")
    //   data-gsap-end-trigger  → seletor do elemento-alvo pra end relativo
    //   data-gsap-pin-spacing  → "true" reserva espaço (quebra overlap)
    //   data-gsap-pin-reparent → "true" move pro <body> durante pin (use raramente)
    //   data-gsap-anticipate   → anticipatePin em segundos
    function initPin() {
        ...
        document.querySelectorAll('.gsap-pin').forEach(function (el) {
            ...
            if (endSelector) {
                var et = document.querySelector(endSelector);
                if (et) { config.endTrigger = et; }
            }
